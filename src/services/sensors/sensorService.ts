services/sensor/sensorservice.ts
...
// PERBAIKAN FIXED: Menghapus filter .eq('land_id') agar data terbaru langsung terambil
export async function fetchLatestSensorReading(
  userId: string, 
  systemType: SystemType,
  landId: LandId, // Tetap dipertahankan agar tidak merusak type-safety komponen UI
): Promise<SensorReading | null> {
  const { data, error } = await supabase
    .from(`sensor_readings_${systemType}`)
    .select('*')
...
// PERBAIKAN FIXED: Menghapus filter .eq('land_id') untuk pencarian data harian
export async function fetchTodaySensorReading(
  ...
  if (error) {
    console.error("Gagal mengambil data harian dari Supabase:", error.message);
    throw error;
  }
  return data ?? null;
}
...
export async function insertSensorReading(
  ...
  const { error } = await supabase
    .from(`sensor_readings_${reading.system_type}`)
...
// PERBAIKAN FIXED: Mengubah pemancar realtime agar tidak menyaring land_id dari alat IoT
export function subscribeToSensorReadings(
  ...
  return supabase
    .channel(`sensor_readings:${systemType}`)
    .on(
      'postgres_changes',
      {
        event: 'INSERT',
        schema: 'public',
        table: `sensor_readings_${systemType}`,
      },
      payload => {
        const next = payload.new as SensorReading;
        // Sekarang hanya memvalidasi apakah tipenya sama-sama portable/panel
        if (next.system_type === systemType) {
          onReading(next);
        }
      },
    )
